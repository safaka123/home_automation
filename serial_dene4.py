...
def zamanlama():
    ...
    while True: 
        ...
        if event == "zamanlama_baslat":
            if(values['dakika']=="" or values['saniye']==""):
                sg.popup("Saat veya dakika boş bırakılamaz!", custom_text="Tamam")
                window3.close()
                return 
            if(values['rolesecim']==""):
                sg.popup("Röle Seçim Yeri Boş Bırakılamaz!", custom_text="Tamam")
                window3.close()
                return 
