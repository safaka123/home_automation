# ARDUINO GERİ NE GÖNDERECEKSE BURAYA FORMATINI YAZ. 
# DOSYALARIN KONUMUNU VE İSMİNİ YAZ. HATTA MİKROBOTİKTE ALINACAK YERİ YAZ. GİTHUB GİBİ. 
...
# Çalışma dizini: D:\2024_DESKTOP\PYTHON_OGREN\serial_2023\mikrobotik_yazi\Serial_2023_PY
# C:\Users\HP\AppData\Roaming\Python\Python310\site-packages\serial\__init__.py

# h5 dosyasını python 3.8 dizininden alıyor. çalışma dizini ayarlanamadı.

ANA_DİZİN = Path(__file__).parent
GİRDİ_DİZİN = ANA_DİZİN / "girdi"
GİRDİ_DİZİN.mkdir(parents=True, exist_ok=True)
MODEL_DİZİN = ANA_DİZİN / "model"
MODEL_DİZİN.mkdir(parents=True, exist_ok=True)

elektrik_veri = pd.read_csv(GİRDİ_DİZİN / "electrical_1.csv") # diğer veri setleri için carton_1.csv ya da clothing_1.csv
loaded_model = load_model(MODEL_DİZİN / "model3.h5")
...
def serial_ports():
    if sys.platform.startswith('win'):  # windows kullanılıyorsa
        ports = ['COM%s' % (i + 1) for i in range(5)] # Hız için 256'dan 32'e düşürüldü.#yukarıdaki satırdaki komut i nin for döngüsüyle artarak array şeklinde yazılmasını sağlıyor. 
